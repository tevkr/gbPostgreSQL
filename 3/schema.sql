...
CREATE INDEX CONCURRENTLY available_udp_ports_availability_idx on available_udp_ports (availability);

-- Получение пользователей, находящихся в конкретном лобби
CREATE INDEX CONCURRENTLY users_lobby_id_idx on users (lobby_id);